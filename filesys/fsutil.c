...
fsutil_rm (char **argv) 
{
  ...
  printf ("Deleting '%s'...\n", file_name);
  if (!filesys_remove (ROOT_DIR_SECTOR, file_name))
  ...
  for (;;)
    {
      ...
      if (type == USTAR_EOF)
      ...
      else if (type == USTAR_REGULAR)
        {
          ...
          if (!filesys_create (ROOT_DIR_SECTOR, file_name, size))
            PANIC ("%s: create failed", file_name);
          dst = filesys_open (ROOT_DIR_SECTOR, file_name);
          if (dst == NULL)
            PANIC ("%s: open failed", file_name);
...
fsutil_append (char **argv)
{
  ...
  /* Open source file. */
  src = filesys_open (ROOT_DIR_SECTOR, file_name);
  if (src == NULL)
    PANIC ("%s: open failed", file_name);
